...
export const StatsStrip: React.FC = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="py-12 sm:py-16 bg-ivory-150/80 dark:bg-charcoal-900 border-b border-ivory-300 dark:border-stone-800 transition-colors relative"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Header Ribbon with DEMO DATA Badge */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-6 mb-8 border-b border-ivory-300 dark:border-stone-800 text-xs">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {stats.map((stat, idx) => (
            <div
              key={stat.id}
              className="p-5 sm:p-6 rounded-xl bg-white dark:bg-charcoal-850 border border-ivory-300 dark:border-stone-800 space-y-3 relative group hover:border-brass-400/60 transition-all shadow-soft-light dark:shadow-none"
            >
              {/* Top Row: Icon + Indicator */}
              <div className="flex items-center justify-between">
                <div
                  className={`w-9 h-9 rounded-lg flex items-center justify-center p-2 border transition-colors ${
                    stat.color === 'burgundy'
              ...
              <div className="space-y-0.5">
                <div className="text-3xl sm:text-4xl font-serif font-normal text-charcoal-800 dark:text-ivory-100 tracking-tight">
                  <AnimatedCounter value={stat.value} isVisible={isVisible} />
                  {stat.suffix}
                </div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-charcoal-800 dark:text-ivory-100 font-mono">
                  {stat.label}
                </h4>
                <p className="text-[11px] font-serif italic text-stone-500 dark:text-stone-400">
                  {stat.hindiLabel}
                </p>
              </div>

              {/* Description */}
              <p className="text-xs text-stone-600 dark:text-stone-400 font-sans font-light leading-relaxed pt-2 border-t border-ivory-200 dark:border-stone-800">
                {stat.description}
              </p>
            </div>
          ))}
