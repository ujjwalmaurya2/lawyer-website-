...
export const CaseTimeline: React.FC<CaseTimelineProps> = ({
  ...
  return (
    <div className={cn('w-full py-3', className)}>
      {/* Horizontal Stage Stepper with Burgundy, Gold, Navy Accents */}
              ...
              <div key={stage} className="flex flex-col items-center text-center relative z-10">
                ...
                >
                  {isCompleted ? (
                    ...
                    <CircleDot className="w-4 h-4 animate-pulse text-brass-300" />
                  ) : (
      ...
      {/* Detailed Chronological Events List with Burgundy & Gold Markers */}
      <div className="relative pl-6 sm:pl-8 border-l-2 border-burgundy-800/40 dark:border-burgundy-900 space-y-5">
        {events.map((event, idx) => (
          <div key={idx} className="relative group">
            ...
            <div
              className={cn(
                'absolute -left-[31px] sm:-left-[39px] top-0.5 w-5 h-5 rounded-full flex items-center justify-center border-2 transition-colors shadow-sm',
                event.completed
            ...
            >
              {event.completed ? (
                ...
                <Clock className="w-3 h-3 text-brass-300" />
              ) : (
